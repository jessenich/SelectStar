        ...
        Try {
            ...
            ElseIf($Notify.IsPresent) {
                $script:Connections.$DestinationConnectionName.BulkLoad($srcReader, $DestinationTable, $ColumnMap, $BatchSize, $BatchTimeout, {
                        ...
                        Write-Progress -Activity "SelectStar BulkCopy" -Status $DestinationTable -CurrentOperation "Inserted $insertCount rows."
                    }.GetNewClosure())
                Write-Progress -Activity "SelectStar BulkCopy" -Status "Finished" -Completed
            }
